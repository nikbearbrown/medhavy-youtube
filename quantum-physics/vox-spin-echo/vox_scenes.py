...
class B08_T2Decay(Scene):
    def construct(self):
        ...
        title = Text("Echo Amplitude Decays as e^(−2τ/T₂)", font=DISPLAY,
        ...
        t2_lbl = Text("T₂ (real spin-spin only)", font=SERIF, font_size=22,
                      color=TEAL, slant=ITALIC).move_to(np.array([2.2, 1.8, 0.0]))
        cancel_lbl = Text("static errors cancel exactly", font=SERIF,
...
class B09_MRI(Scene):
    def construct(self):
        ...
        title = Text("Why MRI Works", font=DISPLAY, font_size=28,
                     color=INK, weight=BOLD).move_to(np.array([0.0, 3.0, 0.0]))
        self.add(title)
        ...
        imperfect_lbl = Text("field uneven by 1 ppm", font=SERIF, font_size=22,
        ...
        t2_result = Text("T₂ measurable — field imperfection irrelevant",
...
class B10_ExampleFit(Scene):
    def construct(self):
        ...
        t2_fit_lbl = Text("T₂ ≈ 360 ms (illustrative)", font=SERIF,
