...
class B11_BobScrambled(Scene):
    def construct(self):
        ...
        # Follow only the selected branch; do not turn this into a full table.
        guide_header = Text("Alice's result", font=SERIF, color=SLATE,
                            font_size=24, slant=ITALIC)
        guide_header.move_to(LEFT * 3.2 + UP * 1.4)
        result = LabelChip("10", accent=SLATE, size=32)
        undo = Text("undo with Z", font=DISPLAY, color=TEAL,
                    font_size=30, weight=BOLD)
        guide_rows = VGroup(result, undo).arrange(DOWN, buff=0.35)
        guide_rows.next_to(guide_header, DOWN, buff=0.3)
...
class B05_BellPairDistribution(Scene):
    def construct(self):
        total = DUR["B05"]
        title = Text("A resource prepared in advance", font=DISPLAY, color=INK,
                     font_size=38, weight=BOLD).to_edge(UP, buff=0.65)
        alice = _person_node("Alice", SLATE).move_to(LEFT * 4 + DOWN * 0.2)
        bob = _person_node("Bob", SLATE).move_to(RIGHT * 4 + DOWN * 0.2)
        qa = _qubit_box("A", TEAL, 1.35, 0.72).next_to(alice, UP, buff=0.3)
        qb = _qubit_box("B", TEAL, 1.35, 0.72).next_to(bob, UP, buff=0.3)
        link = DashedLine(qa.get_right(), qb.get_left(), color=TEAL,
                          stroke_width=3, dash_length=0.18)
        label = Text("shared Bell pair", font=SERIF, color=TEAL,
                     font_size=28, slant=ITALIC).move_to(UP * 0.95)
        note = Text("distributed before the message exists", font=SERIF,
                    color=SLATE, font_size=23, slant=ITALIC).to_edge(DOWN, buff=0.75)
        self.play(FadeIn(title), run_time=0.5)
        self.play(FadeIn(alice, shift=RIGHT * 0.3), FadeIn(bob, shift=LEFT * 0.3), run_time=0.7)
        self.play(FadeIn(qa), FadeIn(qb), Create(link), FadeIn(label), run_time=0.9)
        self.play(FadeIn(note), run_time=0.4)
        self.wait(max(0.5, total - 2.5))


class B13_NoFasterThanLight(Scene):
    def construct(self):
        total = DUR["B13"]
        title = Text("Before the two bits arrive", font=DISPLAY, color=INK,
                     font_size=40, weight=BOLD).to_edge(UP, buff=0.7)
        ball = Circle(radius=1.35).set_fill(SLATE, 0.10).set_stroke(SLATE, 3)
        ball.move_to(RIGHT * 2.7 + DOWN * 0.15)
        center = Dot(ball.get_center(), radius=0.16, color=SLATE)
        bob = Text("Bob's local state", font=SERIF, color=SLATE,
                   font_size=25, slant=ITALIC).next_to(ball, DOWN, buff=0.35)
        mixed = Text("maximally mixed", font=DISPLAY, color=CRIMSON,
                     font_size=30, weight=BOLD).move_to(ball)
        bits = VGroup(LabelChip("?", accent=SLATE, size=27),
                      LabelChip("?", accent=SLATE, size=27)).arrange(RIGHT, buff=0.2)
        bits.move_to(LEFT * 3.6 + UP * 0.2)
        wait_label = Text("no usable information", font=SERIF, color=CRIMSON,
                          font_size=28, slant=ITALIC).next_to(bits, DOWN, buff=0.5)
        self.play(FadeIn(title), run_time=0.5)
        self.play(FadeIn(bits), FadeIn(wait_label), run_time=0.6)
        self.play(Create(ball), FadeIn(center), FadeIn(mixed), FadeIn(bob), run_time=0.8)
        self.wait(max(0.5, total - 1.9))


class B16_Series(Scene):
    def construct(self):
        total = DUR["B16"]
        chip = LabelChip("BEAR'S DOODLES", accent=TEAL, size=24)
        title = Text("Quantum Mechanics", font=DISPLAY, color=INK,
                     font_size=50, weight=BOLD)
        sub = Text("Information · Measurement · Open Systems", font=SERIF,
                   color=SLATE, font_size=29, slant=ITALIC)
        group = VGroup(chip, title, sub).arrange(DOWN, buff=0.42)
        self.play(FadeIn(group, shift=UP * 0.12), run_time=0.9)
        self.wait(max(0.5, total - 0.9))


class B17_TitleOutro(Scene):
    def construct(self):
        total = DUR["B17"]
        bg = Rectangle(width=14.3, height=8.1).set_fill("#171717", 1).set_stroke(width=0)
        title = Text("How to Move a Qubit", font=DISPLAY, color=WHITE,
                     font_size=46, weight=BOLD)
        title2 = Text("Without Ever Copying It", font=DISPLAY, color=WHITE,
                      font_size=46, weight=BOLD)
        byline = Text("Liam, in for Bear", font=SERIF, color="#D7C8FF",
                      font_size=28, slant=ITALIC)
        group = VGroup(title, title2, byline).arrange(DOWN, buff=0.3)
        self.add(bg)
