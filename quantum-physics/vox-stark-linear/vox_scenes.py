...
class B04_FourDegenerate(Scene):
    def construct(self):
        ...
        title = Text("Four Degenerate States at n=2", font=DISPLAY, font_size=28,
                     color=INK, weight=BOLD).move_to(np.array([0.0, 3.0, 0.0]))
        self.add(title)
        ...
        problem_lbl = Text("ordinary perturbation theory → denominator = 0",
...
# ── B06: Parity kills 15 of 16 ───────────────────────────────────────────────

class B06_ParityKills(Scene):
    def construct(self):
        dur = DUR.get("B06", 12.0)

        title = Text("Parity Kills 15 of 16", font=DISPLAY, font_size=28,
                     color=INK, weight=BOLD).move_to(np.array([0.0, 3.0, 0.0]))
        self.add(title)

        cell_size = 1.1
        # Which cell survives: (row=0, col=1) = ⟨2s|z|2p₀⟩
        surviving = (0, 1)
        ...
        parity_lbl = Text("parity kills same-parity pairs", font=SERIF,
                          font_size=20, color=INK, slant=ITALIC
                          ).move_to(np.array([0.0, -2.0, 0.0]))
        survive_lbl = Text("only ⟨2s|z|2p₀⟩ = −3a₀ survives", font=SERIF,
...
class B09_LinearVsQuadratic(Scene):
    def construct(self):
        ...
        title = Text("Degeneracy → Linear;  Unique → Quadratic", font=DISPLAY,
...
class B10_NumericalExample(Scene):
    def construct(self):
        ...
        bright_lbl = Text("middle: ×2 intensity", font=SERIF, font_size=20,
                          color=SLATE, slant=ITALIC).move_to(np.array([-1.5, 0.4, 0.0]))
