...
class B02_BlochIntro(Scene):
    def construct(self):
        ...
        surface_lbl = SerifLabel("surface — pure", accent=TEAL, size=26)
        surface_lbl.move_to(RIGHT * 3.2 + UP * 1.6)

        # Center dot (maximally mixed)
        center_dot = Dot(radius=0.15, color=CRIMSON)
        center_dot.move_to(LEFT * 1.5)
        classical_lbl = SerifLabel("center — maximally mixed", accent=CRIMSON, size=26)
...
class B07_Branching(Scene):
    def construct(self):
        ...
        arrow_length_note = SerifLabel("transverse coherence follows overlap", accent=TEAL, size=24)
...
class B10_Center(Scene):
    def construct(self):
        total = DUR["B10"]
        left = _bloch_circle(radius=1.65, accent=SLATE).move_to(LEFT * 3.5)
        right = _bloch_circle(radius=1.65, accent=SLATE).move_to(RIGHT * 3.5)
        vertical = Arrow(LEFT * 3.5, LEFT * 3.5 + UP * 1.05, color=TEAL,
                         stroke_width=4, buff=0, tip_length=0.2)
        center = Dot(RIGHT * 3.5, radius=0.18, color=CRIMSON)
        l1 = Text("dephasing only", font=DISPLAY, color=TEAL, font_size=28, weight=BOLD)
        l1.next_to(left, DOWN, buff=0.35)
        l2 = Text("populations also equalized", font=DISPLAY, color=CRIMSON,
                  font_size=25, weight=BOLD).next_to(right, DOWN, buff=0.35)
        note1 = Text("can stop on z axis", font=SERIF, color=SLATE,
                     font_size=22, slant=ITALIC).next_to(l1, DOWN, buff=0.2)
        note2 = Text("center = maximally mixed", font=SERIF, color=SLATE,
                     font_size=22, slant=ITALIC).next_to(l2, DOWN, buff=0.2)
        self.play(Create(left), Create(right), run_time=0.8)
        self.play(GrowArrow(vertical), FadeIn(center), run_time=0.7)
        self.play(FadeIn(l1), FadeIn(l2), FadeIn(note1), FadeIn(note2), run_time=0.7)
...
class B13_T2Scale(Scene):
    def construct(self):
        ...
        # Specific dust-grain-in-air estimate — not a universal macroscopic constant.
        bar_w_cat = 0.6
        ...
        lbl_cat = Text("dust grain in air   ~10^-36 s*", font=MONO, color=CRIMSON,
                       font_size=22)
        ...
        caveat = Text("*model- and environment-dependent estimate", font=SERIF,
                      color=SLATE, font_size=19, slant=ITALIC).to_edge(DOWN, buff=0.45)
        self.play(FadeIn(caveat), run_time=0.4)
...
class B15_Example(Scene):
    def construct(self):
        ...
        # T2 marks 1/e coherence, not an expiration time.
        done_chip = LabelChip("T2: 1/e LEFT", accent=CRIMSON, size=22)
        ...
        note_lbl = SerifLabel("error grows continuously — budget fidelity", accent=CRIMSON, size=23)
        ...
        # Simultaneous: cursor sweeps right + transverse arrow shrinks to 1/e.
        reduced = Arrow(LEFT * 3.8 + UP * 0.5,
                        LEFT * 3.8 + UP * 0.5 + RIGHT * (1.8 / np.e),
                        color=CRIMSON, stroke_width=4, buff=0, tip_length=0.18)
        self.play(
            cursor.animate.put_start_and_end_on(tl_end, tl_end + UP * 0.45),
            ReplacementTransform(arr, reduced),
            FadeOut(start_lbl),
            run_time=2.5
        )
...
def _full_card(title, sub, accent=TEAL):
    head = Text(title, font=DISPLAY, color=INK, font_size=46, weight=BOLD)
    if head.width > 11.5:
        head.scale_to_fit_width(11.5)
    detail = Text(sub, font=SERIF, color=accent, font_size=29, slant=ITALIC)
    if detail.width > 11.2:
        detail.scale_to_fit_width(11.2)
    return VGroup(head, detail).arrange(DOWN, buff=0.5)


class B04_Environment(Scene):
    def construct(self):
        total = DUR["B04"]
        q = _bloch_circle(1.45, TEAL).move_to(ORIGIN)
        labels = ["stray field", "neighbor", "vibration"]
        positions = [LEFT * 4.4 + UP * 1.7, RIGHT * 4.4 + UP * 1.7, DOWN * 2.5]
        chips = VGroup(*[LabelChip(x, accent=SLATE, size=23).move_to(p)
                         for x, p in zip(labels, positions)])
        arrows = VGroup(*[Arrow(c.get_center(), ORIGIN, color=SLATE,
                                stroke_width=2, buff=0.55) for c in chips])
        self.play(Create(q), run_time=0.6)
        self.play(LaggedStart(*[FadeIn(c) for c in chips], lag_ratio=0.15),
                  LaggedStart(*[GrowArrow(a) for a in arrows], lag_ratio=0.15), run_time=1.2)
        self.wait(max(0.5, total - 1.8))


class B05_Correlations(Scene):
    def construct(self):
        total = DUR["B05"]
        local = _qubit_box("qubit", TEAL, 2.0, 0.85).move_to(LEFT * 4)
        env = _qubit_box("environment", SLATE, 2.8, 0.85).move_to(RIGHT * 4)
        links = VGroup(*[Line(LEFT * 2.8 + UP * y, RIGHT * 2.4 + UP * y,
                                  color=TEAL, stroke_width=2) for y in (-0.45, 0, 0.45)])
        caption = _full_card("Local information becomes correlation",
                             "the joint state can remain quantum", TEAL).scale(0.72).to_edge(UP, buff=0.55)
        lock = Text("environment control required", font=SERIF, color=CRIMSON,
                    font_size=26, slant=ITALIC).to_edge(DOWN, buff=0.7)
        self.play(FadeIn(caption), FadeIn(local), FadeIn(env), run_time=0.8)
        self.play(Create(links), run_time=0.8)
        self.play(FadeIn(lock), run_time=0.5)
        self.wait(max(0.5, total - 2.1))


class B06_Cheat(Scene):
    def construct(self):
        total = DUR["B06"]
        card = _full_card("The useful model", "and where the shortcut cheats", CRIMSON)
        self.play(FadeIn(card, shift=UP * 0.15), run_time=0.8)
        self.wait(max(0.5, total - 0.8))


class B09_NotAlwaysZero(Scene):
    def construct(self):
        total = DUR["B09"]
        card = _full_card("Zero coherence ≠ zero Bloch vector",
                          "unequal populations leave a z component", CRIMSON)
        z = Arrow(DOWN * 1.7, UP * 1.7, color=TEAL, stroke_width=5, buff=0,
                  tip_length=0.25).next_to(card, DOWN, buff=0.65)
        self.play(FadeIn(card), run_time=0.6)
        self.play(GrowArrow(z), run_time=0.7)
        self.wait(max(0.5, total - 1.3))


class B11_T2Definition(Scene):
    def construct(self):
        total = DUR["B11"]
        card = _full_card("T2 is a timescale", "not a countdown to zero", CRIMSON)
        eq = Text("coherence(T2) = 1/e ≈ 37%", font=MONO, color=TEAL,
                  font_size=38).next_to(card, DOWN, buff=0.75)
        self.play(FadeIn(card), run_time=0.6)
        self.play(FadeIn(eq, shift=UP * 0.15), run_time=0.6)
        self.wait(max(0.5, total - 1.2))


class B14_MeasurementLimit(Scene):
    def construct(self):
        total = DUR["B14"]
        left = _full_card("Decoherence explains", "why interference disappears", TEAL).scale(0.72)
        right = _full_card("It does not explain", "which outcome occurs", CRIMSON).scale(0.72)
        pair = VGroup(left, right).arrange(RIGHT, buff=1.2)
        divider = Line(UP * 2.5, DOWN * 2.5, color=SLATE, stroke_width=2)
        self.play(FadeIn(left, shift=RIGHT * 0.2), Create(divider), run_time=0.8)
        self.play(FadeIn(right, shift=LEFT * 0.2), run_time=0.7)
        self.wait(max(0.5, total - 1.5))


class B16_Engineering(Scene):
    def construct(self):
        total = DUR["B16"]
        items = ["cleaner materials", "colder operation", "shielding", "control pulses", "error correction"]
        chips = VGroup(*[LabelChip(x, accent=TEAL if i == 4 else SLATE, size=23)
                         for i, x in enumerate(items)]).arrange(DOWN, buff=0.27)
        title = Text("Extend coherence — do not confuse the clock", font=DISPLAY,
                     color=INK, font_size=40, weight=BOLD).to_edge(UP, buff=0.6)
        self.play(FadeIn(title), run_time=0.5)
        self.play(LaggedStart(*[FadeIn(c, shift=RIGHT * 0.15) for c in chips], lag_ratio=0.12), run_time=1.3)
        self.wait(max(0.5, total - 1.8))


class B17_Verdict(Scene):
    def construct(self):
        total = DUR["B17"]
        title = Text("THE VERDICT", font=DISPLAY, color=SLATE, font_size=24)
        lines = VGroup(
            Text("Decoherence = information leaves your subsystem", font=SERIF, color=INK, font_size=34),
            Text("T2 leaves 1/e, not zero", font=SERIF, color=TEAL, font_size=34),
            Text("Center requires more than generic dephasing", font=SERIF, color=CRIMSON, font_size=34),
        ).arrange(DOWN, buff=0.42, aligned_edge=LEFT)
        group = VGroup(title, lines).arrange(DOWN, buff=0.65)
        self.play(FadeIn(title), run_time=0.4)
        self.play(LaggedStart(*[FadeIn(x, shift=UP * 0.12) for x in lines], lag_ratio=0.2), run_time=1.2)
        self.wait(max(0.5, total - 1.6))


class B19_TitleOutro(Scene):
    def construct(self):
        total = DUR["B19"]
        bg = Rectangle(width=14.3, height=8.1).set_fill("#171717", 1).set_stroke(width=0)
        title = Text("The Shrinking Arrow", font=DISPLAY, color=WHITE,
                     font_size=54, weight=BOLD)
        sub = Text("How Quantum Becomes Classical", font=SERIF, color="#D7C8FF",
                   font_size=32, slant=ITALIC)
        sign = Text("Liam, in for Bear", font=SERIF, color=WHITE,
                    font_size=26, slant=ITALIC)
        group = VGroup(title, sub, sign).arrange(DOWN, buff=0.35)
        self.add(bg)
