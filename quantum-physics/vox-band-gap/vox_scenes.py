...
class B01_ThreeMaterials(Scene):
    def construct(self):
        dur = DUR.get("B01", 9.0)
...
class B07_GapOpens(Scene):
    def construct(self):
        ...
        title = Text("Potential Energy Difference = Band Gap", font=DISPLAY,
                     font_size=26, color=INK, weight=BOLD
                     ).move_to(np.array([0.0, 3.0, 0.0]))
        self.add(title)
...
class B08_BandStructure(Scene):
    def construct(self):
        ...
        title = Text("Gap = 2|V_G|", font=DISPLAY, font_size=28,
                     color=INK, weight=BOLD).move_to(np.array([0.0, 3.0, 0.0]))
        self.add(title)
        ...
        strong_lbl = Text("stronger lattice\n→ wider gap", font=SERIF, font_size=20,
...
class B09_Classification(Scene):
    def construct(self):
        ...
        title = Text("Why Gaps Classify Materials", font=DISPLAY, font_size=28,
                     color=INK, weight=BOLD).move_to(np.array([0.0, 3.0, 0.0]))
        self.add(title)
...
class B10_SiliconExample(Scene):
    def construct(self):
        dur = DUR.get("B10", 12.0)

        title = Text("Silicon: A = 5.4 Å, Gap = 1.1 eV", font=DISPLAY,
                     font_size=26, color=INK, weight=BOLD
                     ).move_to(np.array([0.0, 3.0, 0.0]))
        self.add(title)

        # Simple schematic of silicon energy levels
        ...
        theory_lbl = Text("|V_G| = gap/2 = 0.55 eV   ← Fourier component",
